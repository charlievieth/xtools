...
func TestTokenOffset(t *testing.T) {
	...
	for _, pkg := range pkgs {
		for ident, obj := range pkg.TypesInfo.Uses {
			...
			// The only permitted use is in github.com/charlievieth/xtools/lsp/source.Offset,
			...
			t.Errorf(`%s: Unexpected use of (*go/token.File).Offset. Please use github.com/charlievieth/xtools/lsp/source.Offset instead.`, fset.Position(ident.Pos()))
		}
	}
}
