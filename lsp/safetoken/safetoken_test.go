...
func TestTokenOffset(t *testing.T) {
	...
	}, "go/token", "github.com/charlievieth/xtools/lsp/...", "golang.org/x/tools/gopls/...")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, pkg := range pkgs {
		...
		case "go/token":
			tokenPkg = pkg
		case "github.com/charlievieth/xtools/lsp/safetoken":
			safePkg = pkg
		}
	}
	...
	if safePkg == nil {
		t.Fatal("missing package github.com/charlievieth/xtools/lsp/safetoken")
	}
	...
	for _, pkg := range pkgs {
		...
		for ident, obj := range pkg.TypesInfo.Uses {
			...
			t.Errorf(`%s: Unexpected use of (*go/token.File).Offset. Please use github.com/charlievieth/xtools/lsp/safetoken.Offset instead.`, fset.Position(ident.Pos()))
		}
	}
}
